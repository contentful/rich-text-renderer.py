...
- **`make docs`** — it runs `rm -rf docs` and replaces `docs/` with generated
  Sphinx HTML. `docs/ADRs/` lives under `docs/`, so this target will delete the
  decision records. There is no `_docs/` directory in the repository, so the
  target cannot succeed as written anyway.
